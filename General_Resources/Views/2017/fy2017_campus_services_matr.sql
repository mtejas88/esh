...
temp as (
  ...
  --counting correctly allocated non fiber circuits to specific campus. the non fiber connections need 1:1 allocation
  (case
    when not(li.connect_category ilike '%Fiber%')
    and li.num_lines != -1
  ...
  --counting clean correctly allocated non fiber circuits to specific campus. the non fiber connections need 1:1 allocation
  (case
    when not(li.connect_category ilike '%Fiber%')
    and li.num_lines != -1
    and ac.num_open_flags = 0
...
the relevant line item and allocation tables to determine which services they receive. Good allocations for fiber
are defined by:
1. num lines >= num recips
2. num lines >= num campuses + other recips
3. num lines >= sum allocations
Good allocations for non-fiber are defined by:
1. num lines = num recips
2. num lines = num campuses + other recips
3. num lines = sum allocations
