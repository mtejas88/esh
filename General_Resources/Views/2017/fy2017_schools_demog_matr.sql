...
charter_to_include as (select charter_esh_id,
  district_esh_id
...
left join charter_to_include 
on eim.entity_id = charter_to_include.charter_esh_id
...
where eim.entity_id is not null /* JAMIE-TEMP-EDIT this removes the 'Unknown' entities, if we want to add them back in we can remove this line */ 
  ...
  and (f.charter__c = false or a.type = 'Charter' 
            or charter_to_include.charter_esh_id is not null)
