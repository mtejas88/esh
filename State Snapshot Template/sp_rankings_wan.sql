...
where wan_conditions_met = true and dirty_status = 'include clean' and shared_service = 'District-dedicated' 
...
ranked as (
  ...
  row_number() over (partition by wc.postal_cd order by num_circuits desc, num_students desc) as ranking

  from wan_clean wc
  left join parent_overall po
  on concat(wc.postal_cd,wc.reporting_name) = concat(po.postal_cd,po.reporting_name)
  left join overall o
  on wc.postal_cd = o.postal_cd
)
