...
sub.hist <- dta.afford[dta.afford$diff.bw.perc > -1,]
...
dta.groups.mean <- aggregate(dta.afford$diff.bw, by=list(dta.afford$group), FUN=mean, na.rm=T)
names(dta.groups.mean) <- c('group', 'mean')
## aggregate median difference of each grouping
dta.groups.median <- aggregate(dta.afford$diff.bw, by=list(dta.afford$group), FUN=median, na.rm=T)
...
for (i in 1:5){
  sub <- dta.afford[dta.afford$group == i,]
...
write.csv(dta.afford, "data/interim/affordability_grouping.csv", row.names=F)
