...
## State Aggregation
state_2017 <- read.csv("data/raw/state_aggregation/2017_state_aggregation_frozen.csv", as.is=T, header=T, stringsAsFactors=F)
state_2016 <- read.csv("data/raw/state_aggregation/2016_state_aggregation_frozen.csv", as.is=T, header=T, stringsAsFactors=F)
...
state_rural_small_town <- read.csv("data/raw/state_aggregation/2017_rural_small_town_state_aggregation_frozen.csv", as.is=T, header=T, stringsAsFactors=F)

## Top Service Providers
top_sp <- read.csv("data/raw/top_service_providers_frozen.csv", as.is=T, header=T, stringsAsFactors=F)
...
## right now, ranking everyone with 100% as 1st and then anyone after as 2nd, 3rd, 4th, etc.
...
state_2017 <- merge(state_2017, sp_state, by='postal_cd', all.x=T)
state_2017 <- merge(state_2017, sp_state_students, by='postal_cd', all.x=T)
## calculate percentage of students affected by top 5 out of overall not meeting
state_2017$students_not_meeting_goals_served_by_sp_top_5_perc_actual_not_meeting <- round(state_2017$num_students_not_meeting_goals_served_by_sp_top_5 / state_2017$students_not_meeting_2014_bw_goal_actual, 2)
## calculate percentage of students affected by top 5 out of overall not meeting extrap
state_2017$students_not_meeting_goals_served_by_sp_top_5_perc_extrap_not_meeting <- round(state_2017$num_students_not_meeting_goals_served_by_sp_top_5 / state_2017$students_not_meeting_2014_bw_goal_extrap, 2)
...
state_2017$district_fiber_targets_extrap <- round((state_2017$clean_district_bw_targets / state_2017$districts_clean_ia_sample)
                                                  * state_2017$districts_population, 0)
## 2017 Campuses not on Fiber (Extrapolated)
...
snapshots <- state_2017[state_2017$postal_cd != 'ALL',c('postal_cd', 'state_name', 'connectivity_ranking', 'erate_money_millions',
                                                        ...
                                                        'students_not_meeting_goals_served_by_sp_top_5_perc_actual_not_meeting',
...
## State Aggregation
write.csv(state_2017_orig, "tool_frozen/data/2017_state_aggregation.csv", row.names=F)
write.csv(state_2016, "tool_frozen/data/2016_state_aggregation.csv",row.names=F)
write.csv(state_2016_froz, "tool_frozen/data/2016_2015_sots_state_aggregation.csv",row.names=F)

## Deluxe Districts
write.csv(dd_2017, "tool_frozen/data/2017_deluxe_districts.csv", row.names=F)
write.csv(dd_2016, "tool_frozen/data/2016_deluxe_districts.csv", row.names=F)
write.csv(dd_2016_froz, "tool_frozen/data/2016_sots_deluxe_districts.csv", row.names=F)

## Click-Throughs
write.csv(current17.click.through, "tool_frozen/data/current17_click_through.csv", row.names=F)
write.csv(sots16.click.through, "tool_frozen/data/sots16_click_through.csv", row.names=F)
write.csv(upgrades.click.through, "tool_frozen/data/upgrades_click_through.csv", row.names=F)
write.csv(connectivity.click.through, "tool_frozen/data/connectivity_click_through.csv", row.names=F)
write.csv(fiber.click.through, "tool_frozen/data/fiber_click_through.csv", row.names=F)
write.csv(affordability.click.through, "tool_frozen/data/affordability_click_through.csv", row.names=F)
write.csv(wifi.click.through, "tool_frozen/data/wifi_click_through.csv", row.names=F)

## Targets
write.csv(connectivity.targets, "tool_frozen/data/connectivity_targets.csv", row.names=F)
write.csv(fiber.targets, "tool_frozen/data/fiber_targets.csv", row.names=F)
...
if (deploy == 1){
  ...
  rsconnect::deployDoc("tool_frozen/2017_Frozen_State_Metrics_Dashboard.Rmd")
}
