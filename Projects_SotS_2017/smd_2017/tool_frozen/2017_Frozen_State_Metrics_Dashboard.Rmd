---
title: "2017 Frozen State Metrics Dashboard"
...
renderValueBox({
  ...
  valueBox(state_name_caps, paste("Date Updated:", date.updated, sep=" "), color="gray")
})
```
...
renderHighchart({
  highchart() %>% 
    ...
    hc_add_series(data = c(state.agg.sots.2016.sub()$sots15_districts_mtg2014goal_perc,
      ...
      round((state.agg.2016.sub()$districts_meeting_2014_bw_goal / state.agg.2016.sub()$districts_clean_ia_cost_sample)*100, 0),
      round((state.agg.2017.sub()$districts_meeting_2014_bw_goal / state.agg.2017.sub()$districts_clean_ia_cost_sample)*100, 0)),
...
renderHighchart({
  highchart() %>% 
    ...
    hc_xAxis(categories = c(paste('Published SotS 2015 (n=', format(state.agg.sots.2016.sub()$sots15_districts_sample, big.mark = ",", nsmall = 0, scientific = FALSE), ')', sep=''),
      paste('Published SotS 2016 (n=', format(state.agg.sots.2016.sub()$current16_districts_sample, big.mark = ",", nsmall = 0, scientific = FALSE), ')', sep=''),
      paste('Current 2016 (n=', format(state.agg.2016.sub()$districts_clean_ia_cost_sample, big.mark = ",", nsmall = 0, scientific = FALSE), ')', sep=''),
      paste('Current 2017 (n=', format(state.agg.2017.sub()$districts_clean_ia_cost_sample, big.mark = ",", nsmall = 0, scientific = FALSE), ')', sep=''))) %>% 
    hc_yAxis(labels = list(format = '{value}%'), min = 0, max = 100) %>% 
    hc_add_series(data = c(state.agg.sots.2016.sub()$sots15_districts_mtg_affordability_perc,
      ...
      round((state.agg.2016.sub()$districts_meeting_affordability / state.agg.2016.sub()$districts_clean_ia_cost_sample)*100, 0),
      round((state.agg.2017.sub()$districts_meeting_affordability / state.agg.2017.sub()$districts_clean_ia_cost_sample)*100, 0)),
...
renderHighchart({
  highchart() %>% hc_chart(type = 'column') %>% 
  hc_xAxis(categories = c('SotS 2016', 'Current 2016', 'Current 2017')) %>% 
  hc_yAxis(labels = list(format = '${value}'), min=0, max=max(state.agg.sots.2016.sub()$c2_remaining_millions_sots,
                                                              state.agg.2017.sub()$c2_remaining_2016/1000000,
                                                              state.agg.2017.sub()$c2_remaining_2017/1000000, na.rm=T)) %>% 
  hc_add_series(data = c(round(state.agg.sots.2016.sub()$c2_remaining_millions_current16, 1),
