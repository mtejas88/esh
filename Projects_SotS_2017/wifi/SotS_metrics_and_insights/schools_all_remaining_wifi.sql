...
select 
sum(budget_remaining_c2_2017_postdiscount) as remaining_budget,
count(school_esh_id) as num_schools_included,
count(distinct district_esh_id) as num_districts,
count(  case 
          when budget_remaining_c2_2017 = c2_budget 
            then school_esh_id 
        end)::numeric / count(school_esh_id)::numeric as perc_schools_used_no_budget,
count(  case 
          when budget_remaining_c2_2017 >= .5 * c2_budget 
            then school_esh_id 
        end)::numeric / count(school_esh_id) as perc_schools_with_at_least_half_funds_remaining
