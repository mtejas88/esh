with wan_bw_2016 as (
  select recipient_id, sum(quantity_of_line_items_received_by_district*bandwidth_in_mbps) as wan_bw
...
dist_2017 as (
	...
	left join wan_bw_2017
	on dd.esh_id = wan_bw_2017.recipient_id
...
select
	...
	case
		...
		when meeting_2014_goal_no_oversub_2016 = false and meeting_2014_goal_no_oversub_2017
			then 'bw upgrade IA new meeting'
		when upgrade_indicator and meeting_2014_goal_no_oversub_2017 = false
		  	then 'bw upgrade IA and not meeting'
		when upgrade_indicator
			then 'bw upgrade IA'
		when wan_bw_10pct_more
			then 'bw upgrade WAN only'
		else 'no bw increase'
