...
application_funding_by_category as (
	...
	where postal_cd = '{{two_letter_state}}' or 'All' = '{{two_letter_state}}'
),
...
-- calculated only for those districts where num_students is known
	sum(case when num_students is not null then ben_category_2_cost end) / sum (num_students) as "Average C2 Cost per Student",
--	median(case when ben_category_2_cost_per_student is not null then ben_category_2_cost_per_student end) as "Median C2 Cost per Student",
-- calculated only for those districts where num_students is known
...
--	,
--	sum(case when num_students is not null then ben_category_2_cost end)/1000000 as "Known Students: Total C2 E-Rate Cost ($M)",
--	sum (num_students) * 150 / 1000000 as "Known Students: Total C2 E-Rate Budget ($M)",
--	sum (num_students) * 150 / 1000000 - sum(case when num_students is not null then ben_category_2_cost end)/1000000 as "Known Students: Total C2 E-Rate Budget Remaining"
