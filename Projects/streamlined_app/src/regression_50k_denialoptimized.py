...
#remove outliers
mean= np.mean(lowcost_50k_applications_c1['total_monthly_eligible_recurring_costs'])
std = np.std(lowcost_50k_applications_c1['total_monthly_eligible_recurring_costs'])

lowcost_50k_applications_c1 = lowcost_50k_applications_c1.loc[abs(lowcost_50k_applications_c1['total_monthly_eligible_recurring_costs'] - mean) < 3 * std]
...
est_50k_den = sm.Logit(y_res, X_res.astype(float)).fit()
print(est_50k_den.summary())
...
yhat_test = est_50k_den.predict(x_test)
...
np.savetxt('data/interim/sm_lowcost_50k_applications_c1_denial_optimized.out', cm, delimiter=',') 

## predict denial optimized with 2016
x0  = lowcost_50k_applications.loc[lowcost_50k_applications['category_1'] == True]
x = x0[feature_cols]
x = sm.add_constant(x)

yhat = est_50k_den.predict(x.T.drop_duplicates().T)
#plt.hist(yhat,100)
#plt.show()
...
x0.to_csv('data/interim/lowcost_50k_applications_c1_2016_denial_optimized.csv')

x0_summ = x0.groupby('yhat').agg({'total_funding_year_commitment_amount_request': 'sum', 'application_number': 'count'})

x0_summ.to_csv('data/interim/summ_lowcost_50k_applications_c1_2016_denial_optimized.csv')
...
x1.to_csv('data/interim/lowcost_50k_applications_c1_2017_denial_optimized.csv')

x1_summ = x1.groupby('yhat').agg({'total_funding_year_commitment_amount_request': 'sum', 'application_number': 'count'})

x1_summ.to_csv('data/interim/summ_lowcost_50k_applications_c1_2017_denial_optimized.csv')
