...
## data prep
#import
lowcost_110k_applications = pd.read_csv('data/interim/lowcost_110k_applications.csv')

lowcost_110k_applications_c1  = lowcost_110k_applications.loc[lowcost_110k_applications['category_1'] == True]
...
#define model inputs
train, test = train_test_split(lowcost_110k_applications_c1, train_size=0.75, random_state=1)
...
est_110k_app = sm.Logit(y_res, X_res.astype(float)).fit()
print(est_110k_app.summary())
...
yhat_test = est_110k_app.predict(x_test)
...
np.savetxt('data/interim/sm_lowcost_110k_applications_c1_approval_optimized.out', cm, delimiter=',') 

## predict denial optimized with 2016
x0  = lowcost_110k_applications_c1
x = x0[feature_cols]
x = sm.add_constant(x)

yhat = est_110k_app.predict(x.T.drop_duplicates().T)
#plt.hist(yhat,100)
#plt.show()
...
x0.to_csv('data/interim/lowcost_110k_applications_c1_2016_approval_optimized.csv')

x0_summ = x0.groupby('yhat').agg({'total_funding_year_commitment_amount_request': 'sum', 'application_number': 'count'})

x0_summ.to_csv('data/interim/summ_lowcost_110k_applications_c1_2016_approval_optimized.csv')


## 2017 data prep
#import
lowcost_110k_applications_2017 = pd.read_csv('data/interim/lowcost_110k_applications_2017.csv')


## predict denial optimized with 2017
x1  = lowcost_110k_applications_2017.loc[lowcost_110k_applications_2017['category_1'] == True]
x = x1[feature_cols]
x = sm.add_constant(x)
...
x1.to_csv('data/interim/lowcost_110k_applications_c1_2017_approval_optimized.csv')

x1_summ = x1.groupby('yhat').agg({'total_funding_year_commitment_amount_request': 'sum', 'application_number': 'count'})

x1_summ.to_csv('data/interim/summ_lowcost_110k_applications_c1_2017_approval_optimized.csv')
