...
## read in data 
speKC.includedApps <- read.csv("data/interim/special_construction_applications_0417.csv", as.is=T, header=T)
speKC.frns <- read.csv("data/interim/frns_with_inclusion_reasons_0417.csv", as.is=T, header=T)
...
write.csv(speKC.includedAppsAgg, file = "data/interim/special_construction_decisions_0417.csv")
...
prop.table(table(speKC.includedAppsAgg$finalDecision))[2]/
  (prop.table(table(speKC.includedAppsAgg$finalDecision))[2]+prop.table(table(speKC.includedAppsAgg$finalDecision))[3])
