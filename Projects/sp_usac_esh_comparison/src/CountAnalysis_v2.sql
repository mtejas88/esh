...
SELECT
  ...
  count(distinct case 
                  ...
                  AND (esh.name = usac.sp_name 
                  AND esh.reporting_name = usac.dba)
                    then esh.id else null end) as no_matching_spin_matching_name_reportingname,
  count(distinct case
                  when esh.spin is null
                  AND esh.name = usac.sp_name
                  AND esh.reporting_name != usac.dba
                    then esh.id else null end) as no_matching_spin_matching_name,
  count(case
                  when esh.spin is null
                  AND esh.reporting_name = usac.dba
                  AND esh.name != usac.sp_name
                    then esh.id else null end) as no_matching_spin_matching_reportingname
  FROM public.esh_service_providers esh
  RIGHT JOIN public.usac_sp_spin_v2 usac
    ON esh.name = usac.sp_name