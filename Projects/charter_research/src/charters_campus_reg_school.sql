with schools_demog_adj as (
  ...
  where eim.entity_id is not null /* JAMIE-TEMP-EDIT this removes the 'Unknown' entities, if we want to add them back in we can remove this line */ 
    ...
    --only looking at charter schools
    --and (f.charter__c = false or a.type = 'Charter')

),

charters_same_campus as (

select
  s.district_esh_id,
  s.campus_id,
  count(case
          when s.school_type = 'Charter'
            then s.school_esh_id
        end) as num_charters,
  count(case
          when s.school_type = 'Traditional'
            then s.school_esh_id
        end) as num_traditional,
...
select distinct unnest(charters) as charter_esh_id
from charters_same_campus

)

select *
from temp
where charter_esh_id is not null