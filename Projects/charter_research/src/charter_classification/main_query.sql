...
-- note this will create duplicate records for entities that have more than one ben mapped to them
	select distinct e.entity_id as entity_esh_id,
...
rs_campus as (select distinct entity_campus_id,
...
serv as (select u.applicant_ben,
		--applicant info
...
charter_recip as (select cs_1.charter_esh_id,
