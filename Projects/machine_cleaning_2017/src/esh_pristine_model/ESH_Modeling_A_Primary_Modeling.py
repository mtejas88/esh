...
## pass in the model, data.columns.tolist() (2016 data)
visualize_tree()
...
validation_set = md[md.frn_complete.isin(validation_set.frn_complete)]
md = md[-md.frn_complete.isin(validation_set.frn_complete)]
print md.shape
...
X = md.drop(['frn_complete','cl_connect_category'],axis=1) ## Drop some cols from the X data
...
vld_X = vld_X.drop(['frn_complete','cl_connect_category'],axis=1)
...
vld_predict_probabilites.index = validation_set['frn_complete']
vld_predict_probabilites.reset_index().to_csv('model_data_versions/June21_validation_set_lit_fiber_probabilities.csv'                                              ,index=False,float_format='%.7f')
...
joblib.dump(MCC_Predict_Model, 'final_models/general_multiclass.pkl') 


# In[50]:

joblib.dump(binaryLF_Predict_Model, 'final_models/lit_fiber_binary_classifier.pkl')
