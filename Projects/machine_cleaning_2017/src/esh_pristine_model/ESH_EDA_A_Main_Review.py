...
# # Summary
# 
# In this file, ESH adapted the DK code to review the transformed and flagged raw 2016 data.
# 
# Previous Note by DK: In this file, DK reviewed the raw data. Not all exploration is recoreded but things that inlfuenced featurization are included. We used this notebooks a sort of exploration scratch pad - there may not be immediate need for ESH to review and use this notebook but some of the code or summaries may prove useful. 
...
## Method Commented Out Grabs Everything That is Unique
#metadata[cols]
...
## append a "cl_" prefix to all clean columns
...
df_duplicates = clean.groupby(['cl_frn_complete']).size().reset_index().rename(columns={0:'count_duplicates'})
df_duplicates = df_duplicates[df_duplicates.count_duplicates >= 2]
print df_duplicates.shape

clean = clean[-clean.cl_frn_complete.isin(df_duplicates.cl_frn_complete)]
print clean.shape


# In[11]:

## merge in clean connect category on id
#full_mg = pd.merge(full_mg, clean[clean.cl_exclude == False][['cl_id','cl_connect_category']], left_on='id',right_on='cl_id',how='inner')
#full_mg = pd.merge(full_mg, clean[['cl_frn_complete','cl_connect_category']], left_on='frn_complete',right_on='cl_frn_complete',how='inner')

full_mg = pd.merge(mg_raw, clean[['cl_frn_complete','cl_connect_category']], left_on='frn_complete', right_on='cl_frn_complete', how='inner')
full_mg.shape
...
full_mg.cl_connect_category.nunique()
...
narrative_test = full_mg[['frn','purpose','narrative','cl_connect_category']]
...
connect_classes = [x for x in connect_classes if pd.notnull(x)]
...
# NOTE: Not needed anymore since we are using the transformed data and the column is gone
...
# **Notes**: 
# 1. This category looks useful in spotting the ISP only records: Internet access service with no circuit (data circuit to ISP state/regional network is billed separately) 
...
sns.distplot(full_mg[full_mg.upload_bandwidth_in_mbps.notnull()].upload_bandwidth_in_mbps)
print full_mg.upload_bandwidth_in_mbps.describe()
print sum(full_mg.upload_bandwidth_in_mbps.isnull())
...
sns.distplot(full_mg[full_mg.bandwidth_in_mbps.notnull()].bandwidth_in_mbps)
print full_mg.bandwidth_in_mbps.describe()
print sum(full_mg.bandwidth_in_mbps.isnull())
...
# 
# NOTE: Speeds are normalized in the new data
...
#full_mg[full_mg.std_download_spd > 0].groupby('cl_connect_category').agg(\
#                                {'monthly_recurring_unit_costs':['min','max','mean','median'],
...
#ipynb_convert.executeConvertNotebook('ESH_EDA_A_Main_Review.ipynb', 'ESH_EDA_A_Main_Review.py', main)
