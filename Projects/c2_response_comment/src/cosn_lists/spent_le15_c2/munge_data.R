...
spent_le15_c2 <- read.csv("spent_le15_c2.csv", as.is=T, header=T, stringsAsFactors=F)
...
spent_le15_c2$name = tolower(spent_le15_c2$name)
spent_le15_c2$name <- gsub("[[:punct:]]", "", spent_le15_c2$name)
...
match_indicator_column <- function(col) {
  ...
  for(i in 1:nrow(cosn)){
    sub=spent_le15_c2[spent_le15_c2$postal_cd == cosn[i,"postal_cd"],]
...
#spent_le15_c2$first2words=getwords(spent_le15_c2)
spent_le15_c2$first2words=gsub(" ", "", spent_le15_c2$name, fixed = TRUE)
