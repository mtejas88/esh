/*
Author: Multiple Analysts
Created On Date: Q4 2016
Last Modified Date: 02/02/2016 
Name of QAing Analyst(s): Justine Schott 
Purpose: Districts table data pull with added columns for:
...
Updated version includes dirty districts w/o ia_cost_per_mbps and ia_bandwidth_per_student values
*/
...
select *,
case when ia_bandwidth_per_student is not null then (ia_bandwidth_per_student::numeric/1000) * num_students::numeric else null end as total_ia_bw_mbps, 
case when ia_bandwidth_per_student is not null and annual_ia_cost_per_mbps is not null then (ia_bandwidth_per_student::numeric/1000) * num_students::numeric * annual_ia_cost_per_mbps::numeric/12 else null end as total_ia_monthly_cost,
case when ia_bandwidth_per_student is not null and ia_bandwidth_per_student::numeric >= 100 then true
else false end as meeting_2014_goal_no_oversub,
case when ia_bandwidth_per_student is not null and ia_bandwidth_per_student::numeric * ia_oversub_ratio >= 100 then true
else false end as meeting_2014_goal_oversub,
case when ia_bandwidth_per_student is not null and ia_bandwidth_per_student::numeric >= 1000 then true
else false end as meeting_2018_goal_no_oversub,
case when ia_bandwidth_per_student is not null and ia_bandwidth_per_student::numeric * ia_oversub_ratio >= 1000 then true
else false end as meeting_2018_goal_oversub,
case when monthly_ia_cost_per_mbps is not null and monthly_ia_cost_per_mbps<=3 then true else false end as meeting_$3_per_mbps_affordability_target,
...
from 
  ...
  case when districts.ia_cost_per_mbps!='Insufficient data' and districts.ia_cost_per_mbps!='Infinity' then districts.ia_cost_per_mbps else null end as "annual_ia_cost_per_mbps",
  case when districts.ia_cost_per_mbps!='Insufficient data' and districts.ia_cost_per_mbps!='Infinity' then districts.ia_cost_per_mbps::numeric/12 else null end as "monthly_ia_cost_per_mbps",
