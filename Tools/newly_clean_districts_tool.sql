...
nc as (select *,
case when exclude_from_analysis=false and (status_before_change='0' or status_before_change='created') 
then 'Yes' else 'No' end as "newly_clean"
from sc)

select entity_id,
max(districts.name) as "name",
max(districts.postal_cd) as "postal_cd",
0 as "num_open_dirty_flags",
count(item_id) as "num_flags_resolved",
array_agg(nc.label) as "flags_resolved",
