...
left join (
  ...
  from (
        select 
          ...
          sum(case 
                when sr.connect_category = 'Fiber' 
                  then sr."cat.1_allocations_to_district"
                else 0
              end) as fiber_lines,
          sum(case 
                when sr.connect_category = 'Fixed Wireless' 
                  then sr."cat.1_allocations_to_district"
                else 0
              end) as fixed_wireless_lines,
          sum(case 
                when sr.connect_category not in ('Fiber', 'Other / Uncategorized') 
                  then sr."cat.1_allocations_to_district"
                else 0
              end) as non_fiber_lines,
          sum(case 
                when sr.connect_type = 'Cable Modem' 
                  then sr."cat.1_allocations_to_district"
                else 0
              end) as cable_lines,
          sum(case 
                when (sr.connect_type != 'Cable Modem' and sr.connect_category in ('Copper', 'Cable / DSL'))
                      OR sr.open_flags ilike '%ethernet_copper%'
                  then 
          ...
          sum(case 
                when  isp_conditions_met=false 
                      ...
                      and not(sr.open_flags ilike '%ethernet_copper%')
                  then sr."cat.1_allocations_to_district"
                else 0
              end) as other_uncategorized_lines,
          sum(case 
                when sr.wan_conditions_met = true 
                  then sr."cat.1_allocations_to_district"
                else 0
              end) as wan_lines,
          sum(case 
          ...
          sum(case 
                when  (internet_conditions_met = true or upstream_conditions_met = true) 
                      and (sr.connect_type != 'Cable Modem' and connect_category in ('Cable / DSL', 'Copper'))
                          OR sr.open_flags ilike '%ethernet_copper%'
                  then sr."cat.1_allocations_to_district"
                else 0 
          ...
          sum(case 
                when  (internet_conditions_met = true or upstream_conditions_met = true) 
                      and sr.connect_type = 'Cable Modem'
                  then sr."cat.1_allocations_to_district"
                else 0 
